...
import { formatEnrolmentStatusDisplay, formatCurrencyOr, getAvatarColor, getInitials, getTaskStatusLabel, navGuard } from '../utils/helpers';
...
type DetailsPartnerRsrc = {
  ...
  dynamicProperties?: Partial<DetailsPartnerRsrc>;
};

type DetailsPartnerListRsrc = {
  participantPin?: number | string | null;
  scenarioNumber?: number | string | null;
  inCombinedFarm?: boolean | number | string | null;
  combinedFarmNumber?: number | string | null;
  enrolmentPartnerList?: DetailsPartnerRsrc[] | null;
  dynamicProperties?: Partial<DetailsPartnerListRsrc>;
};

type DetailsPartnerRow = {
  operation: string;
...
type DetailsCombinedFarmSummary = {
  participantPin: string;
  combinedFarmNumber: string;
  scenarioNumber: string;
};

function getDetailsStringField(record: unknown, field: string): string {
...
function mergeDetailsPartnerDynamic<T extends { dynamicProperties?: Partial<T> }>(value: T): T {
  return value.dynamicProperties ? { ...value, ...value.dynamicProperties } : value;
}

function getDetailsPartnerListResponse(response: unknown): DetailsPartnerListRsrc | null {
  if (!response || typeof response !== 'object') return null;
  return mergeDetailsPartnerDynamic(response as DetailsPartnerListRsrc);
}

function getDetailsPartnerRows(response: unknown): DetailsPartnerRsrc[] {
  const listResponse = getDetailsPartnerListResponse(response);
  return (listResponse?.enrolmentPartnerList ?? [])
    .filter((row): row is DetailsPartnerRsrc => !!row && typeof row === 'object')
    .map(mergeDetailsPartnerDynamic);
}

function formatDetailsPartnerPercent(value: unknown): string {
...
function getDetailsNumberValue(value: unknown): number {
  const numberValue = Number(value);
  return Number.isFinite(numberValue) ? numberValue : Number.NEGATIVE_INFINITY;
}

function getDetailsBooleanFieldValue(value: unknown): boolean {
  return value === true || value === 1 || value === '1' || String(value).toLowerCase() === 'true';
}

function buildDetailsPartnerRows(response: unknown): DetailsPartnerRow[] {
  const rows = getDetailsPartnerRows(response)
    .map(row => ({
      scenarioNumber: getDetailsNumberValue(row.scenarioNumber),
      operation: String(row.operationSchedule ?? '').trim(),
      operationPartnershipPin: String(row.operationPartnershipPin ?? '').trim(),
      partnerParticipantPin: String(row.partnerParticipantPin ?? '').trim(),
      partnerPercent: formatDetailsPartnerPercent(row.partnerPercent),
      firstName: String(row.firstName ?? '').trim(),
      lastName: String(row.lastName ?? '').trim(),
      partnershipName: String(row.partnershipName ?? '').trim(),
  ...
  for (const row of rows) {
    const key = [row.operation, row.operationPartnershipPin, row.partnerParticipantPin, row.firstName, row.lastName, row.partnershipName].join('|');
  ...
  return [...latestByPartner.values()].map(({ scenarioNumber: _sn, operationPartnershipPin: _op, ...row }) => row);
}

function getDetailsCombinedFarm(response: unknown): DetailsCombinedFarmSummary | null {
  const listResponse = getDetailsPartnerListResponse(response);
  if (!listResponse || !getDetailsBooleanFieldValue(listResponse.inCombinedFarm)) return null;
  return {
    participantPin: String(listResponse.participantPin ?? '').trim(),
    combinedFarmNumber: String(listResponse.combinedFarmNumber ?? '').trim(),
    scenarioNumber: String(listResponse.scenarioNumber ?? '').trim(),
  };
}

async function getDetailsAccountPinFromXrm(accountId: string): Promise<string> {
  const candidates = [window, window.parent, window.top];
  for (const candidate of candidates) {
    try {
      if (!candidate) continue;
      const retrieveRecord = (candidate as unknown as DetailsXrmWebApiHost).Xrm?.WebApi?.retrieveRecord;
      if (!retrieveRecord) continue;
      const result = await retrieveRecord('account', accountId, '?$select=vsi_pin,accountnumber,name');
      let pin = getDetailsStringField(result, 'vsi_pin');
      if (!pin) pin = getDetailsStringField(result, 'accountnumber');
      return pin;
    } catch {
...
async function getDetailsAccountPin(accountId: string): Promise<string> {
  ...
  let pin = getDetailsStringField(genericAccount.data, 'vsi_pin');
  if (!pin) pin = getDetailsStringField(genericAccount.data, 'accountnumber');
  if (pin) return pin;
  return getDetailsAccountPinFromXrm(accountId);
}

function getDetailsProgramYear(record: Vsi_participantprogramyears | null): number | null {
...
export function EnrolmentDetailsPage() {
  ...
  const [partnerRows, setPartnerRows] = useState<DetailsPartnerRow[]>([]);
  const [combinedFarmSummary, setCombinedFarmSummary] = useState<DetailsCombinedFarmSummary | null>(null);
  ...
  // Resolve participant PIN for FARMS API calls
  useEffect(() => {
    ...
    let cancelled = false;
    getDetailsAccountPin(participantId)
      .then(pin => { if (!cancelled) setParticipantPin(pin); })
      .catch(() => { if (!cancelled) setParticipantPin(''); });
    return () => { cancelled = true; };
  }, [record]);

  // Fetch partner and combined farm data from FARMS API
  useEffect(() => {
    const programYear = getDetailsProgramYear(record);
    ...
    farmsApi.getEnrolmentPartners<DetailsPartnerListRsrc>(participantPin, farmsScenarioProgramYear)
      .then(result => {
        if (cancelled) return;
        if (!result.success) throw new Error(result.error?.message ?? 'Unable to load FARMS enrolment partners.');
        setPartnerRows(buildDetailsPartnerRows(result.data));
        setCombinedFarmSummary(getDetailsCombinedFarm(result.data));
      })
      .catch(err => {
        if (cancelled) return;
        setPartnerRows([]);
        setCombinedFarmSummary(null);
        const msg = err instanceof Error ? err.message : 'Unable to load FARMS enrolment partners.';
        setPartnerRowsError(msg.includes('404') ? 'No ENW scenario has been created in FARMS.' : msg);
      })
      .finally(() => { if (!cancelled) setPartnerRowsLoading(false); });

    return () => { cancelled = true; };
  }, [participantPin, record]);
  ...
  return (
    <section className="details-wrapper">
      ...
      <div className="details-composite">
        ...
        <div className="details-section-break" />
        <div className="details-content-section">
          <h2 className="details-section-heading">Partnerships &amp; Combined Partners</h2>
          {partnerRowsLoading && <p className="details-partner-state">Loading partner data...</p>}
          {partnerRowsError && <p className="details-partner-state details-partner-state-error">{partnerRowsError}</p>}
          {!partnerRowsLoading && !partnerRowsError && !combinedFarmSummary && partnerRows.length === 0 && (
            <p className="details-partner-state">No partner or combined farm data found.</p>
          )}
          {!partnerRowsLoading && !partnerRowsError && combinedFarmSummary && (
            <div className="details-combined-farm">
              <h3 className="details-combined-farm-heading">Combined Farm</h3>
              <table className="details-partner-table">
                <thead>
                  <tr>
                    <th scope="col">PIN</th>
                ...
                <tbody>
                  <tr>
                    <td>{combinedFarmSummary.participantPin || '-'}</td>
                    <td>{combinedFarmSummary.combinedFarmNumber || '-'}</td>
                    <td>{combinedFarmSummary.scenarioNumber || '-'}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          )}
          {!partnerRowsLoading && !partnerRowsError && partnerRows.length > 0 && (
                ...
                .map(row => {
                  ...
                  return (
                    <div
                      ...
                      key={`${row.operation}-${row.partnerParticipantPin}-${row.firstName}-${row.lastName}`}
                    >
                      <div className="details-partner-card-top">
                        <div className="details-partner-name">{displayName || '-'}</div>
                        {row.partnerParticipantPin && (
                          <div className="details-partner-pin">PIN {row.partnerParticipantPin}</div>
                        )}
                      </div>
                      <dl className="details-partner-details">
                        <div>
                          <dt>Operation</dt>
                          <dd>{row.operation || '-'}</dd>
                        </div>
                        ...
                        <div>
                          <dt>Enrolment Fee</dt>
                          <dd>{formatCurrencyOr(row.enrolmentFee, '-')}</dd>
                        </div>
                        <div className="details-partner-name-row">
                          <div>
                            <dt>First Name</dt>
                            <dd>{row.firstName || '-'}</dd>
                          </div>
